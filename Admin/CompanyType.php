...
//Toa add Company type
	if(isset($_POST["btn_submit"]))
	{
    //Values tbl_company_type
	$name=$_POST["txt_name"];
	$insqry="INSERT INTO tbl_company_type(company_type_name) values('".$name."')";
...
//To delete values
			if(isset($_GET["clrID"]))
			{
			$quer="DELETE FROM tbl_company_type WHERE company_type_id='".$_GET["clrID"]."'";
			if($con->query($quer))
			{
?>
        <script>
...
<form id="form1" name="form1" method="post" action="">
  <div class="form-container">
    <div class="form-header">
      <i class="fas fa-edit"></i> Add Type
    </div>
    <div class="form-body">
      <div class="form-group">
        <label for="txt_name2" class="form-label">Company Type Name</label>
        <input type="text" name="txt_name" id="txt_name2" class="form-control" placeholder="Enter Type" required />
      </div>
...
<!-- All Types -->
   <div class="table-container">
                <div class="table-header">
                    ...
                    <i class="mdi mdi-check-circle-outline"></i> Company Types List
                </div>
   ...
   <?php
   $i=0;
   $selqry="select * from tbl_company_type";
   ...
   while($data=$rows->fetch_assoc())
   {
	...
	?>
   							<tr>
     						<td><?php echo $i ?></td>  
     						<td><?php echo $data['company_type_name'] ?></td>
      						 <td><a href="Companytype.php? clrID=<?php echo $data['company_type_id']?>">Delete</a></td>

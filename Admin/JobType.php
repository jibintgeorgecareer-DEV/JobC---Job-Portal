...
if(isset($_POST["btn_submit"]))
{
 ...
 $insqry="INSERT INTO tbl_job_type(job_type_name) values('".$name."')";
...
<?php
		if(isset($_GET["clrID"])) //delte job type
		{
	$quer="DELETE FROM tbl_job_type WHERE 	job_type_id='".$_GET["clrID"]."'";
	if($con->query($quer))
	{
		?>
        <script>
...
<form id="form1" name="form1" method="post" action="">
  <div class="form-container">
    <div class="form-header">
      <i class="fas fa-edit"></i> Add Job Type
    </div>
    <div class="form-body">
      <div class="form-group">
        <label for="txt_name2" class="form-label">Job Type</label>
        <input type="text" name="txt_name" id="txt_name2" class="form-control" placeholder="Type Name" required />
      </div>
...
<i class="mdi mdi-check-circle-outline"></i> Job Type List
</div>
...
<table border="1" height="150" class="table">
   ...
   <?php
   $i=0;
   $selqry="select * from tbl_job_type ";
   ...
   while($data=$rows->fetch_assoc())
   {
	...
	<tr>
        ...
        <td><?php echo $i ?></td>	
        <td><?php echo $data['job_type_name'] ?></td>
        <td><a href="JobType.php? clrID=<?php echo $data['job_type_id']?>">Delete</a></td>
</tr>
   <?php
   }
   ?>
